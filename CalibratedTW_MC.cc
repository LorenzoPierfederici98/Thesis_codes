...
#include "CalibratedTW_MC.h"

// main
void CalibratedTW_MC(TString infile = "testMC.root", Bool_t isMax = kFALSE, Int_t nev = 10, TString outfile = "AnaFOOT.root", TString outDir = "/Users/marco/FOOT/Analisi/shoe/build/Reconstruction/OutputMacro")
...
void BookHistograms(TDirectory *DirElossLayerX, TDirectory *DirElossLayerY, TDirectory *DirTofLayerX, TDirectory *DirTofLayerY)
{
  ...
  for (int ilay = 0; ilay < kLayers; ilay++)
  {
    for (int ibar = 0; ibar < (int)nBarsPerLayer; ibar++)
    {
      ...
      eloss_true_perBar[ilay][ibar] = new TH1D(Form("Eloss_%s_bar%d", LayerName[(TLayer)ilay].data(), ibar), Form("Eloss true MC %s bar%d", LayerName[(TLayer)ilay].data(), ibar), 300, -10., 20.);
      noCuts_eloss_true_perBar[ilay][ibar] = new TH1D(Form("noCuts_Eloss_%s_bar%d", LayerName[(TLayer)ilay].data(), ibar), Form("No cuts Eloss true MC %s bar%d", LayerName[(TLayer)ilay].data(), ibar), 300, -10., 20.);
